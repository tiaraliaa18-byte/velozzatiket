...
<div class="train-list-wrapper">
    ...
    <div id="train-list-container">
        ...
        <div id="btn-pilih-kursi-container" style="display: none;">
            <button type="button" id="btn-pilih-kereta" onclick="gulingKeKursi()">
                Pilih Kursi
            </button>
        </div>
...
function updateTombolPilihKursi() {
    var btn = document.getElementById('btn-pilih-kereta') || document.querySelector('.btn-pilih-kursi-kamu');
    if (!btn) return;
...
function triggerSortPrice(el, order) {
    ...
    // Ini mengunci agar kartu diurutkan di atas, dan tombol tetap berada di paling bawah kontainer
    cards.forEach(card => {
        container.appendChild(card);
    });
    
    // 6. Jika ada tombol Pilih Kursi atau pembungkusnya di dalam kontainer tersebut, 
    // kita paksa pindahkan kembali ke urutan paling bawah setelah kartu selesai diurutkan.
    const btnPilih = document.getElementById('btn-pilih-kereta') || document.querySelector('.btn-pilih-kursi-kamu');
    if (btnPilih) {
        // Cari pembungkus tombol jika ada class bootstrap seperti mt-4 atau text-center
        const btnWrapper = btnPilih.closest('.text-center, .mt-4') || btnPilih;
...
function selTrain(el, name, cls, dep, arr, price, dur, stops) {
    ...
    // 3. LOGIKA TOMBOL (Penyempurnaan)
    // Cari tombol dan container-nya
    var btnPilih = document.getElementById('btn-pilih-kereta') || document.querySelector('.btn-pilih-kursi-kamu');
    var containerBtn = document.getElementById('btn-pilih-kursi-container'); // Pastikan ID ini ada di HTML
    ...
    if (btnPilih) {
        ...
        btnPilih.style.display = 'block'; // Memastikan tombol terlihat
    }

    // Jika ada container pembungkus tombol, pastikan dia muncul
    if (containerBtn) {
...
function validasiDanLanjut() {
    ...
    allInputs.forEach(function(input) {
        // Cek apakah elemen ini atau container-nya terlihat (display bukan none)
        // .closest('.card') mencari elemen induk berupa card
        var parentCard = input.closest('.card');
        
        // Kita hanya validasi jika:
        // 1. Elemennya terlihat (bukan display: none)
        // 2. ATAU jika itu adalah elemen Kontak Pemesan (yang mungkin tidak dibungkus .card)
    ...
    // Jika sampai di sini, artinya semua input yang terlihat sudah diisi
    goPage('page-confirm', 4);
}

function simpanDanTampilkanData() {
    // 1. Ambil data dari input form (Contoh untuk Penumpang 1)
    var nama = document.getElementById('p1-nama').value;
    var nik = document.getElementById('p1-nik').value;

    // 2. Tampilkan ke halaman konfirmasi
    document.getElementById('konf-nama-pax').textContent = nama;
    document.getElementById('konf-nik-pax').textContent = nik;

    // 3. Generate Kode Booking sementara (untuk tampilan saja sebelum masuk DB)
    var kode = 'VEL-' + Math.floor(1000 + Math.random() * 9000); 
    document.getElementById('konf-kode-booking').textContent = kode;
}

function kirimDataPemesanan() {
    // Kumpulkan data dari input (sesuaikan ID-nya dengan yang ada di form kamu)
    let formData = {
        ...
        id_penumpang: document.getElementById('id_penumpang').value,
        _token: '{{ csrf_token() }}' // PENTING: Untuk keamanan Laravel
    };
    ...
    .then(data => {
        if(data.status === 'success') {
            // Jika berhasil, arahkan ke halaman konfirmasi dengan membawa data kode booking
...
// Fungsi yang dijalankan saat klik "Lanjut ke Konfirmasi"
function simpanDataKeSession() {
    ...
    sessionStorage.setItem('temp_pax_data', JSON.stringify(data));
    
    // Pindah halaman
    window.location.href = '/halaman-konfirmasi';
}
</script>
</body>
</html>